...
module.exports = class ServerlessCweSnsLambda {
    ...
    validateConfig(funcName, stage, config) {
                  ...
                  processEvent:
                    ...
                    events:
                      - cweSns:
                          ...
                          topicResourceName: string                      #required
                          dlqResourceName:  string                        #optional
                          Input || InputPath || InputTransformer: Object  #optional                         
        ...
        return {
            ...
            ruleMessage: config.Input || config.InputPath || config.InputTransformer || {},
    ...
    addSNSTopic(template, {funcName, topicResourceName, prefix}) {
        if (topicResourceName) {
            template.Resources[`${topicResourceName}Topic`] = {
                Type: "AWS::SNS::Topic",
                Properties: {
                    TopicName: `${prefix}${topicResourceName}Topic`
                }
            };
        }
    }
    ...
    addSNSDLQ(template, {prefix, dlqResourceName}) {
        if (!template.Resources[dlqResourceName]) {
            template.Resources[dlqResourceName] = {
                Type: "AWS::SQS::Queue",
                Properties: {
                    QueueName: `${prefix}SNSDeadLetterQueue`,
    ...
    addTopicToCloudWatchRule(template, {topicResourceName, ruleResourceName, ruleMessage, prefix}) {
        if (!template.Resources[ruleResourceName]) {
            throw new Error(`Couldn't find cloudwatch rule ${ruleResourceName}`);
        } else {
    ...
    addSNStoDLQPolicy(template, {topicResourceName, prefix, dlqResourceName}) {
        if (!template.Resources["SNStoDLQInsertPolicy"]) {
            template.Resources["SNStoDLQInsertPolicy"] = {
                Type: "AWS::SQS::QueuePolicy",
                Properties: {
                    PolicyDocument: {
                        Version: "2012-10-17",
                        Id: `${prefix}SNStoDLQInsertPolicy`,
                        Statement: []
                    },
                    Queues: []
                }
        ...
        template.Resources["SNStoDLQInsertPolicy"].Properties.Queues.push({
            Ref: dlqResourceName
        });
        template.Resource["SNStoDLQInsertPolicy"].Properties.PolicyDocument.Statement.push({
            Effect: "Allow",
            Principal: {
    ...
    addCWEtoSNSPolicy(template, {prefix, topicResourceName}) {
        if (!template.Resources["CWEtoSNSInsertPolicy"]) {
            template.Resources["CWEtoSNSInsertPolicy"] = {
        ...
        template.Resources["CWEtoSNSInsertPolicy"].Properties.Topics.push({
            Ref: topicResourceName
        });
        template.Resource["CWEtoSNSInsertPolicy"].Properties.PolicyDocument.Statement.push({
            Effect: "Allow",
            Principal: {
